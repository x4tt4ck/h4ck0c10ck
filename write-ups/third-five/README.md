...
# 4 Tripolar
Нам дается функция crow, которая применяется к (p, q, r) а затем к (_enc * pk, pk * _hash, _hash * _enc) для шифрования сообщения
```python3
def crow(x, y, z):
    return (x**3 + 3*(x + 2)*y**2 + y**3 + 3*(x + y + 1)*z**2 + z**3 + 6*x**2 + (3*x**2 + 12*x + 5)*y + (3*x**2 + 6*(x + 1)*y + 3*y**2 + 6*x + 2)*z + 11*x) // 6
```
Единственный подход здесь - это найти способ получить назад (x, y, z) от crow. Хотя эта функция выглядит сложной, ее можно упростить до гораздо более приятной версии.

Пусть `a = x + y + z + 1`, `b = x + y + 1`. С нашими знаниями, изученными в старшей школе, crow может быть упрощена как:
```
crow = (a**3 + 3*b**2 + 2*b - 6*y - z - 6) // 6
```
Теперь, поскольку x, y, z большие числа, мы можем получить их обратно приближением:
* a примерно равна cuberoot(6 * crow)
* b примерно равна sqrt((6 * crow - a ** 3) / 3) (когда x, y, z недостаточно велики, это значение отличается на единицу)

Тогда мы можем вычислить остаток, чтобы получить (x, y, z)
Полный код решения:
```python3
from Crypto.Util.number import *
...
```
Флаг: `ASIS{I7s__Fueter-PoLy4__c0nJ3c7UrE_iN_p4Ir1n9_FuNCT10n}`

# 5 Merry-go-round
Это задача на реверс, поэтому давайте добавим ее в Гидру. Быстрый анализ подводит нас к основной функции FUN_00101bba. Она делает следующее:
* Инициализирует некоторые таблицы постоянных ключей шифрования
* Обрабатывает файл флага вводя по одному символу за раз, затем шифруя его (см. Функцию FUN_001025f)
* Снова шифрует всю зашифрованную строку (в функции FUN_0010250), на этот раз блоком
* Выводит зашифрованный флаг в файл

Инициализацию таблицы ключей шифрования анализировать не стоит, мы можем просто отладить программу и скопировать таблицу из памяти.

FUN_001025f выполняет xorит код символа для шифрования с некоторыми константами и данными из таблицы ключей. FUN_0010250 работает аналогичным образом, но также включает случайное значение, которое также будет закодировано в конце выходного файла.

Чтобы получить флаг, мы обращаем процесс вспять: получаем случайное значение из закодированного флага, эмулируем реверс FUN_0010250, а затем эмулируем реверс FUN_001025f.

Давайте сделаем python скрипт:
```python3
import binascii
